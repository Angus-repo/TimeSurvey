...
/**
 * Windows 時區名稱 → IANA 時區識別碼的對照表。
 *
 * Microsoft Graph 的信箱設定（mailboxSettings/timeZone）預設回傳 Windows 時區名稱
 * （例如 "Taipei Standard Time"），但前端需要 IANA 識別碼（例如 "Asia/Taipei"）
 * 才能以 Intl API 計算 UTC 偏移、與填寫者的瀏覽器時區比較。
 * 對照內容取自 Unicode CLDR windowsZones 的預設（territory 001）對應。
 */
final class WindowsTimeZones {

    private WindowsTimeZones() {}

    /** 轉成 IANA 時區識別碼；本身已是有效 IANA 識別碼則原樣回傳，查無對應回傳 null */
    static String toIana(String windowsName) {
        if (windowsName == null || windowsName.isBlank()) return null;
        String name = windowsName.trim();
        String mapped = MAP.get(name);
        if (mapped != null) return mapped;
        // Graph 也可能直接回 IANA 識別碼（信箱設定用 IANA 名稱時）
        try {
            if (name.contains("/")) return ZoneId.of(name).getId();
    ...
    private static final Map<String, String> MAP = Map.ofEntries(
            ...
            Map.entry("Mountain Standard Time (Mexico)", "America/Mazatlan"),
            Map.entry("Mountain Standard Time", "America/Denver"),
            Map.entry("Yukon Standard Time", "America/Whitehorse"),
            ...
            Map.entry("Greenland Standard Time", "America/Nuuk"),
            ...
            Map.entry("South Sudan Standard Time", "Africa/Juba"),
